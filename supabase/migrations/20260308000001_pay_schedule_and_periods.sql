...
ALTER TABLE distributors ADD COLUMN IF NOT EXISTS pay_schedule jsonb;
...
CREATE INDEX IF NOT EXISTS idx_pay_periods_account_status ON pay_periods(account_id, status);
CREATE INDEX IF NOT EXISTS idx_pay_periods_period_end ON pay_periods(period_end);
...
ALTER TABLE commissions ADD COLUMN IF NOT EXISTS pay_period_id uuid REFERENCES pay_periods(id);
